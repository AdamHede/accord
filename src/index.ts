...
export class GameRoom extends DurableObject<Env> {
  ...
  async initializeRoom(roomCode: string, name: unknown): Promise<RoomResponse> {
    ...
    const now = Date.now();
    const player: StoredPlayer = { id: crypto.randomUUID(), token: crypto.randomUUID(), name: normalizeName(name), faction: null, joinedAt: now };
    const game: Game = {
      roomCode,
  ...
  async joinGame(name: unknown): Promise<RoomResponse> {
    const game = await this.readGame();
    if (!game) throw new Error("That council does not exist.");
    if (game.status !== "lobby") throw new Error("The council has already started.");
    if (game.players.length >= FACTIONS.length) throw new Error("This council is full.");
    const player: StoredPlayer = { id: crypto.randomUUID(), token: crypto.randomUUID(), name: normalizeName(name), faction: null, joinedAt: Date.now() };
    game.players.push(player);
    game.activity.unshift({ id: crypto.randomUUID(), text: `${player.name} entered the council.`, createdAt: Date.now() });
  ...
  private async startGame(game: Game, player: StoredPlayer): Promise<void> {
    if (player.id !== game.hostPlayerId) throw new Error("Only the convener can begin the council.");
    if (game.players.length < 2) throw new Error("At least two envoys are required.");
    if (game.players.some((candidate) => !candidate.faction)) throw new Error("Every envoy must choose a faction first.");
    game.status = "orders";
  ...
  private async submitOrders(game: Game, player: StoredPlayer, submitted: unknown): Promise<void> {
    ...
    game.activity = game.activity.slice(0, 8);
    if (game.players.every((candidate) => game.orders[candidate.id])) resolveTurn(game, () => crypto.randomUUID());
  }
  ...
  private async sendChat(game: Game, player: StoredPlayer, body: unknown, recipientId: unknown): Promise<void> {
    ...
    if (recipient && !game.players.some((candidate) => candidate.id === recipient)) throw new Error("Unknown recipient.");
    ...
    try {
      ...
      if (roomMatch) {
        ...
        if (action === "join" && request.method === "POST") {
          const body = await requestBody(request);
          if (!body) return error("Expected a JSON request body.");
          return json(await room.joinGame(body.name), 201);
        }
